...
select year , count(id) as movies_produced_in_India_or_USA from movie 
where year = 2019 and country in ( 'India' or 'USA' ) ;
...
from movie m inner join genre g on m.id = g.movie_id inner join ratings using (movie_id)
where year = 2017 and monthname(date_published) = 'March' and country = 'USA' and total_votes > 1000
group by genre
order by movie_count desc ;
...
from movie m inner join ratings r on m.id = r.movie_id 
where median_rating = 8 and date_published between '2018/04/01' and '2019/4/1' ; 
...
FROM names n INNER JOIN director_mapping d ON n.id = d.name_id INNER JOIN ratings r ON r.movie_id = d.movie_id INNER JOIN genre g ON g.movie_id = d.movie_id,
top_genre
...
select production_company , total_votes as vote_count , rank() over( order by total_votes desc ) as prod_camp_rank
...
select name as actor_name , total_votes , count(movie_id) as movie_count , round(sum(total_votes * avg_rating)/sum(total_votes),2) as actor_avg_rating ,
dense_rank() over ( order by round(sum(total_votes * avg_rating)/sum(total_votes),2) desc , total_votes desc ) as actor_rank
from movie m inner join ratings r on m.id = r.movie_id
             inner join role_mapping using (movie_id)
             inner join names on role_mapping.name_id = names.id
...
select name as actress_name , total_votes , count(movie_id) as movie_count , round(sum(total_votes * avg_rating)/sum(total_votes),2) as actress_avg_rating ,
dense_rank() over ( order by round(sum(total_votes * avg_rating)/sum(total_votes),2) desc , total_votes desc ) as actress_rank
from movie m inner join ratings r on m.id = r.movie_id
             inner join role_mapping using (movie_id)
             inner join names on role_mapping.name_id = names.id
...
select title , avg_rating , case when avg_rating > 8 then 'superhit'
								 when avg_rating between 7 and 8 then 'Hit movies'
                                 when avg_rating between 5 and 7 then 'One-time-watch movies'
                                 when avg_rating < 5 then 'Flop movies' end as avg_rating_category
from movie m inner join ratings r on m.id = r.movie_id ;                                
...
select production_company , count(id) as movie_count , row_number() over (order by count(id) desc ) as prod_camp_rank
from movie m inner join ratings r on m.id = r.movie_id 
